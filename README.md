...
正则表达式（英文：Regular Expression，简写为Regex），使用单个字符串来描述、匹配一系列符合某个句法规则的字符串。通常被用来检索、替换那些匹配某个模式的文本。
...
| [xyz] | 字符集，匹配方括号中包含的任意字符，如[abc]；方括号内英文句号表示原有含义，不是元字符；字符集内使用连字符可指定字符范围，如`[a-z]`； |
...
| (?!pattern) | 正向否定预查：在任何不匹配pattern的字符串开始处匹配查找字符串。这是一个非获取匹配。当我们需要指定第一部分表达式的后面不跟随某一内容时，使用负向先行断言。负向先行断言的定义跟我们定义的正向先行断言一样， 唯一的区别在于我们使用否定符号 ! 而不是等号 =，例如 `(?!...)`。 我们来看看下面的正则表达式 `(T|t)he(?!\sfat)`，表示：从输入字符串中获取全部 The 或者 the 且不匹配 fat 前面加上一个空格字符。 |
| (?<=pattern) | 反向肯定预查，与正向肯定预查类似，只是方向相反。正向后行断言用于获取跟随在特定模式之后的所有匹配内容。正向后行断言表示为 `(?<=...)`。例如正则表达式 `(?<=(T|t)he\s)(fat|mat)`，表示：从输入字符串中获取在单词 The 或 the 之后的所有 fat 和 mat 单词。 |
| (?<!pattern) | 反向否定预查，与正向否定预查类似，只是方向相反。负向后行断言是用于获取不跟随在特定模式之后的所有匹配的内容。负向后行断言表示为 `(?<!...)`。例如正则表达式 `(?<!(T|t)he\s)(cat)`，表示：在输入字符中获取所有不在 `The` 或 `the` 之后的所有单词 `cat`。 |
| x\|y | 分支或结构，匹配符号之前的字符或后面的字符。没有包围在()里，其范围是整个正则表达式。 |
| * | 匹配前面的子表达式零次或多次 |
| + | 匹配前面的子表达式一次或多次 |
| ? | 匹配前面的子表达式零次或一次 |
...
| ^ | 定位符，表示行开始，检查匹配字符是否是起始字符 |
| $ | 定位符，表示行结束，检查匹配字符是否是输入字符串的最后一个字符 |
| i | 标记，不区分大小写：将匹配设置为不区分大小写，如`/The/gi`可以匹配`the`； |
| g | 标记，全局搜索：搜索整个输入字符串中的所有匹配，不会在查找到第一个匹配时就停止，如`/The/gi` |
| m | 标记，多行匹配：会匹配输入字符串每一行。如果希望每一行都使用定位符，所以需要使用 `m` 修饰符。 |


## 工具
* [正则表达式可视化 - 1](https://regexper.com/)  
* [正则表达式可视化 - 2](https://jex.im/regulex)
* [RegexGolf](https://alf.nu/RegexGolf) - 练习正则表达式
* [regex101: build, test, and debug regex](https://regex101.com/) - 在自己指定的字符串上测试正则表达式
...
- [cdoco/learn-regex-zh](https://github.com/cdoco/learn-regex-zh)
